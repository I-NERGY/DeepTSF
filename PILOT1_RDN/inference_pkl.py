...
class _MLflowPKLDartsModelWrapper:
...
def _load_pyfunc(model_file):
    ...
    model = load_model(model_uri=f'{model_file}/_model.pkl', mode="local", model_type="pkl")
    scaler = load_scaler(scaler_uri=f"{model_file}/scaler_series.pkl", mode="local")

    return _MLflowPKLDartsModelWrapper(model, scaler)
