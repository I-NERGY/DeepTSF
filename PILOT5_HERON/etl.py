...
@click.option("--series-csv", 
    type=str, 
    default="../../HERON/heron_new/EVB-P1840070_Heron_HQ_Parking.csv"
...
def etl(series_csv, resolution, time_covs):
    ...
    with mlflow.start_run() as mlrun:
        ...
        ts["Duration (h)"] = (ts["End date"] - ts["Start date"]) / np.timedelta64(1, 's') / 3600
        ts = ts.drop(columns=["Duration"])
        ts["Mean Demand (kw)"] = ts["kWh"] / ts["Duration (h)"]

        enter_power = pd.DataFrame(data=ts[["Start date", "Mean Demand (kw)"]])
        enter_power = enter_power.set_index("Start date")
        exit_power = pd.DataFrame(data=ts[["End date", "Mean Demand (kw)"]])
        exit_power["Mean Demand (kw)"] = - exit_power["Mean Demand (kw)"]
        exit_power = exit_power.set_index("End date")
        demand = pd.concat([enter_power, exit_power], axis=0, join='inner').sort_index()
        print(demand)
...
# print df2.head(5)
        ...
        exit_power.to_csv('features/series.csv')
